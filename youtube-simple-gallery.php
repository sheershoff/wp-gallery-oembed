    ...
    switch ( $column ) {
    	case 'ysg_image':
			...
			if ( has_post_thumbnail()) { 
				...
				echo '<img title="'. get_the_title().'" alt="'. get_the_title().'" src="'. get_post_meta($post_id, 'valor_thumbnail_url', true) .'" width="150" height="90" />';	
			}
			echo '</a>';
            break;
...
/**
 * Attempts to fetch the data for a provided URL using oEmbed.
 *
 * @uses _wp_oembed_get_object()
 * @uses WP_oEmbed::get_provider()
 * @uses WP_oEmbed::fetch()
 *
 * @param string $url The URL that should be embedded.
 * @param array $args Additional arguments and parameters.
 * @return bool|string False on failure or the oEmbed data on success.
 */
function oembed_gallery_get_oembed_data( $url, $args = '' ) { // @TODO: rename and change desc
...
/**
 * Attempts to fetch the thumbnail for a provided URL using oEmbed.
 *
 * @uses _wp_oembed_get_object()
 * @uses WP_oEmbed::get_provider()
 * @uses WP_oEmbed::fetch()
 ...
 * @param string $url The URL that should be embedded.
 * @param array $args Additional arguments and parameters.
 * @return bool|string False on failure or the thumbnail url on success.
 */
function oembed_gallery_get_oembed_thumbnail( $url, $args = '' ) { // @TODO: rename and change desc
	$data = oembed_gallery_get_oembed_data($url, $args);
        if($data && isset($data->thumbnail_url) && $data->thumbnail_url!=''){
