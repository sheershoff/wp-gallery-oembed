		...
		<li class="li-YoutubeGallery" id="post-<?php the_ID(); ?>">
			...
			<div id="video-id-<?php the_ID(); ?>" style="display:none;">
                            <?php
                                ...
                                //print_r($thepost);
                                //echo '-->';
                                if(trim($thepost->post_content)){
                            ?>
                                <span><?=$thepost->post_content?></span>
                            <?php
                                }else{
                                    if(false){
                            ?>
                                    <iframe width="<?php echo $ysg_settings['ysg_size_wight']; ?>" height="<?php echo $ysg_settings['ysg_size_height']; ?>" src="http://www.youtube.com/embed/<?php echo $embed_code;?>?autoplay=<?php echo $ysg_settings['ysg_autoplay']; ?>" frameborder="0" allowfullscreen></iframe>
                            <?php
                                    }else{
                                        echo get_post_meta(get_the_ID(), 'valor_embedhtml', true);
                                    }
                                }
                            ?>
                        </div>
			<a href="#TB_inline?width=<?php echo $ysg_settings['ysg_size_wight']; ?>&height=<?php echo $ysg_settings['ysg_size_height']; ?>&inlineId=video-id-<?php the_ID(); ?>" title="<?php the_title();?>" class="thickbox">
				<?php if ( has_post_thumbnail()) { the_post_thumbnail('chr-thumb-youtube', array('class' => 'img-YoutubeGallery chr-size-thumb')); }else{ echo '<img src="'. get_post_meta(get_the_ID(), 'valor_thumbnail_url', true) .'" class="img-YoutubeGallery chr-size-thumb" alt="'.get_the_title().'" title="'.get_the_title().'" />'; } ?>
			</a>
