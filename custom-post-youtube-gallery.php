<?php
	...
	function ysg_meta_box_meta_video(){
		...
		$metaBoxDesc = get_post_meta($post->ID, 'valor_desc', true);
                ...
                $oEmbedHtml = wp_oembed_get($metaBoxUrl,array('width'=>640,'height'=>354));
                $oEmbedThumbnail = oembed_gallery_get_oembed_thumbnail($metaBoxUrl);
                
                //print_r(array('$oEmbedHtml'=>$oEmbedHtml,'$oEmbedThumbnail'=>$oEmbedThumbnail));
		...
		<ul>
			...
			<li>
				<em style="padding: 5px 0; display: block; color: #666;"> '.
					...
					&bull; http://youtu.be/UzifCbU_gJU<br />
                                        &bull; <a href="http://codex.wordpress.org/Embeds#Okay.2C_So_What_Sites_Can_I_Embed_From.3F">other examples</a><br />
				</em>
			</li>';
			if($oEmbedThumbnail != ''){
			?>
			<li>
				...
				<img src="<?php echo($oEmbedThumbnail); ?>" />
				<hr/>
                                <h4><?php echo __('Embed Original do V&iacute;deo', 'youtube-simple-gallery') ;?>:</h4>
                                <?php echo($oEmbedHtml); ?>
                                <br />
	...
	function ysg_save_video_post(){
	    global $post;        
            ...
            $metaBoxUrl = $_POST['valor_url'];
                
            $oEmbedHtml = wp_oembed_get($metaBoxUrl,array('width'=>640,'height'=>354));
            
            if($oEmbedHtml) update_post_meta($post->ID, 'valor_embedhtml', $oEmbedHtml);
            
            $oEmbedThumbnail = oembed_gallery_get_oembed_thumbnail($metaBoxUrl);
            
            if($oEmbedThumbnail) update_post_meta($post->ID, 'valor_thumbnail_url', $oEmbedThumbnail);
